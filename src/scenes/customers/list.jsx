...
import { auth, customerService } from '../../services/firebase-service';
...
const { fetchAllCustomers, fetchLinkedCustomersList } = customerService;
...
export const CustomerList = () => {
  ...
  const getContacts = async () => {
    ...
    try {
      const [ownContactsSnapshot, linkedContactsSnapshot] = await Promise.all([
        fetchAllCustomers(),
        fetchLinkedCustomersList(user.uid),
      ]);
  ...
  const addNewItem = () => {
    navigation.navigate(screens.CustomerDetails);
  };
  ...
  const subrenderItem = ({ item, index }) => (
    <ContactItem
      ...
      item={item}
      onPress={() => {
        navigation.navigate(screens.CustomerDetails, { item, index });
      }}
    />
  );
