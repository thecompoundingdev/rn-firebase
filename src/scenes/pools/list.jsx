...
import { PoolItem } from './list-item';
...
export const PoolList = ({ fetchData, hideCreate, title }) => {
  ...
  useEffect(() => {
    navigation.addListener('focus', () => {
      fetchData().then(data => handleDocSnapshots(data));
    });
  }, []);
  ...
  const addNewItem = () => {
    navigation.navigate(screens.PoolDetails);
  };
  ...
  const subrenderItem = ({ item, index }) => (
    <PoolItem
      ...
      item={item}
      onPress={() => {
        navigation.navigate(screens.PoolDetails, { item, index });
      }}
    />
  );
