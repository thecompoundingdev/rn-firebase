...
export const JobDetails = () => {
  ...
  const updateJob = async () => {
    ...
    const jobPayload = {
      pool: state.contactId,
      customer: state.createdBy,
      maintenanceEmployee: user.uid,
      maintenanceDate: serverTimestamp(),
    };

    try {
      setState(prev => ({ ...prev, isLoader: true }));
      await Promise.all([
        poolService.updatePool(state.contactId, poolPayload),
        jobService.addNewJob(jobPayload),
      ]);
            ...
            >
              {({
                ...
                <>
                  ...
                  <Button
                    disabled={state.isLoader || state.isCompleted}
                    onPress={handleSubmit}
                    size="large"
                    style={styles.createButton}
                  >
                    {state.isCompleted ? 'COMPLETED' : 'COMPLETE'}
                  </Button>
                </>
              )}
            </Formik>
