...
import { newEmployeeSchema } from '../../utils/validations';
...
export const EmployeeDetails = () => {
  ...
  const createContact = async values => {
    ...
    try {
      ...
      await employeeService.addNewEmployee(newContact);
  ...
  const updateContact = async values => {
    ...
    try {
      setState(prev => ({ ...prev, isLoader: true }));
      await employeeService.updateEmployee(state.contactId, payload);
      goBack();
    } catch (ex) {
  ...
  const deleteContact = async () => {
    try {
      setState(prev => ({ ...prev, isLoader: true }));
      await employeeService.deleteEmployee(state.contactId);
      goBack();
    } catch (ex) {
  ...
  return (
    <SafeAreaLayout style={styles.safeArea} insets="top">
      <KeyboardAvoidingView>
        <TopNavigation
          title={existingItem ? 'Employee details' : 'New Employee'}
